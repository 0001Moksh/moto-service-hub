...
CREATE TABLE bike (
    ...
    engine VARCHAR(50),                    -- Fixed spelling from "enginee"
    model_no VARCHAR(50),
...
ALTER TABLE shop 
ADD COLUMN holiday_calendar_id INTEGER REFERENCES holiday_calendar(holiday_calendar_id) ON DELETE SET NULL;
...
-- 8. WORKER_AVAILABILITY (added primary key for Supabase best practice)
...
-- 11. JOB (fixed shope_id → shop_id)
CREATE TABLE job (
    job_id SERIAL PRIMARY KEY,
    bike_id INTEGER REFERENCES bike(bike_id) ON DELETE CASCADE,
    shop_id INTEGER REFERENCES shop(shop_id) ON DELETE CASCADE,   -- Fixed
    worker_id INTEGER REFERENCES worker(worker_id) ON DELETE SET NULL,
    customer_id INTEGER REFERENCES customer(customer_id) ON DELETE CASCADE,
    issue_from_customer TEXT,
...
CREATE TABLE document (
    ...
    document_by_role VARCHAR(50),   -- owner / worker / customer / admin
    role_id INTEGER                 -- references the respective id
);
...
CREATE INDEX idx_worker_shop ON worker(shop_id);
CREATE INDEX idx_bike_customer ON bike(owner_id);
CREATE INDEX idx_booking_customer ON booking(customer_id);
CREATE INDEX idx_booking_shop ON booking(shop_id);
CREATE INDEX idx_job_worker ON job(worker_id);
CREATE INDEX idx_service_worker ON service(worker_id);
